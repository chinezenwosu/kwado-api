...
const EditorProvider = function() {
  ...
  const _authorise = async () => {
    try {
      ...
      const isAuthorised = _kwadoc.users.find((user) => user.user._id.toString() === currentUserId)
      return !!isAuthorised
    }
    catch (e) {
  ...
  const saveToCache = async (payload, errorCallback) => {
    const { data } = payload
    await redis.hSet(getDocumentHash(_documentId), 'content', JSON.stringify(data))
    await redis.hSet(getDocumentHash(_documentId), 'updatedAt', Date.now())
    
    debounce.submit(async () => {
      const isAuthorised = await _authorise()
