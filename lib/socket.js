...
const SocketProvider = function() {
  const setupWSConnection = (socket) => {
    socket.on(socketEmissions.GET_DOCUMENT, async (documentId) => {
      const data = await EditorProvider.loadDocument(documentId)
      ...
      socket.on(socketEmissions.SEND_DOCUMENT_CONTENT_CHANGES, (data) => {
        ...
        EditorProvider.saveToCache(documentId, data.payload.data)
      })
  
      socket.on(socketEmissions.SEND_DOCUMENT_SELECTION, (data) => {
        const dataWithClientId = { ...data, clientId: socket.id }
        socket.broadcast.to(documentId).emit(socketEmissions.RECEIVE_DOCUMENT_SELECTION, dataWithClientId)
      })
  
      socket.on('disconnect', () => {
        socket.broadcast.to(documentId).emit(socketEmissions.CLIENT_DISCONNECTED, socket.id)
      })
    })
  }
